...
function Monetization({ children }) {
  ...
  useEffect(() => {
    const checkUserLogin = async () => {
      try {
        // sleep 1 second
        const userResponse = await fetch(`${API_BASE_URL}/auth/apps/${manifestConfig.appId}/me`, {
          credentials: 'include'
        })

        if (userResponse.ok) {
          const userData = await userResponse.json()
          console.log('userData', userData.app_id)
          console.log('manifestConfig.appId', manifestConfig.appId)
          if (userData.app_id === manifestConfig.appId) {
    ...
    const fetchConfig = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/apps/${manifestConfig.appId}/config`, {
          credentials: 'include'
        })
